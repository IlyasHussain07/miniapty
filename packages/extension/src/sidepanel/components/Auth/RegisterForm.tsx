...
export function RegisterForm() {
  const { register, isLoading, setView } = useStore();
  ...
  return (
    <div style={wrap}>
      ...
      <form onSubmit={async e => { e.preventDefault(); await register(email, password); }} style={form}>
        ...
        <label style={lbl}>Password <span style={{ fontWeight: 400, color: '#a0aec0' }}>(min 8 characters)</span>
      ...
      <p style={footer}>
        Have an account?{' '}
        <button onClick={() => setView('login')} style={link}>Sign in</button>
      </p>
    </div>
  );
}
